...
- Event Details enables an interactive stack back swipe from the left 50pt/dp edge only; do not widen it or add a touch layer over the page `ScrollView` (both stopped vertical scrolling on Android). `patches/@react-navigation+stack+7.6.12.patch` (applied by the `postinstall` `patch-package` hook) maps the stack pan's gesture-handler v1 `minOffsetX`/`maxDeltaY` to v2 `activeOffsetX`/`failOffsetY`; gesture-handler v2 silently drops the v1 names, so without the patch the stack pan activates on any drag direction and cancels native scrolling wherever it receives the touch. Re-create the patch on every `@react-navigation/stack` upgrade (`stackGestureActivation.test.ts` fails if it is missing). `HostRequestTabs` fails its pan immediately for touches beginning in the edge strip and `blocksExternalGesture`s the stack pan (from `GestureHandlerRefContext`), because Android also offers pager touches to the stack pan: RNGH measures its edge hitSlop against the overflowing two-page row. Interior horizontal drags therefore always change Requests/Members (or Requests/Accepted) tabs and vertical drags fail early to the outer `ScrollView`. Keep the stack gesture and pager boundary tied to `EVENT_DETAILS_BACK_EDGE_WIDTH`.
