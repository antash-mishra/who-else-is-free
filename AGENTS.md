...
- `EventsContext.hasLoadedEvents` records whether the first events request has settled, including empty and failed responses. Discover may show its full-page loader only while this is false; focus/background refreshes must preserve an already-resolved empty or populated scene.
...
- Shared event-list primitives live in `src/components/events`; use `EventSectionList`, `EventListPage`, `EventListLoadState`, and `eventListSections` helpers before duplicating event card lists, loading/error presentation, date grouping, or infinite-scroll footer wiring in screens. `EventSectionList` computes `Placed` entry indices across all sections so the first-screen animation cap remains global; never pass the section-local row index through directly.
...
- `EventDetailsScreen` retains its last ready event snapshot for the lifetime of the route, so a successful delete can remove the context row without replacing Details with the not-found fallback before its navigation reset runs. Back from a scrolled card-origin Details page first restores the hero to the top, then starts the shared return; keep that order or the offscreen hero forces a rough fallback transition.
