...
- Event metadata punctuation: use `EVENT_INFO_SEPARATOR` from `src/constants/display.ts` for compact, non-sentence values joined in event cards and event-detail metadata; keep normal prose and saved address punctuation unchanged.
