...
func TestSingleEventJoinRequest(t *testing.T) {
	...
	t.Run("approver sees the new private conversation as read", func(t *testing.T) {
		// The intro is persisted as the requester's message and the join
		// announcement follows it; both were generated by the host's approval,
		// so the host must not see the conversation as unread.
		resp := env.doRequest(t, http.MethodGet, "/api/conversations", avaToken, nil)
		if resp.StatusCode != http.StatusOK {
			t.Fatalf("expected 200, got %d", resp.StatusCode)
		}
		...
		for _, convo := range payload.Conversations {
			...
			if convo.UnreadCount != 0 {
				t.Fatalf("approver unread_count = %d, want 0", convo.UnreadCount)
			}
		}
		if !found {
			t.Fatalf("expected conversation %d in approver's list", conversationID)
		}
	})
