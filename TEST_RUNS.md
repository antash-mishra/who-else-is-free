...
## 2026-09-20 — Issue #156 notification fallback performance

- Change: an empty events response now completes Discover's initial-load lifecycle, so focus refreshes preserve the resolved empty pager instead of replacing it with a full-screen loader. Unavailable notification taps return to Discover before presenting the result sheet, and inbox row/unread reconciliation waits until navigation interactions settle. The transition scheduler has a guarded one-second fallback for a permanently busy interaction queue, and the shared overlay is memoized with stable Discover callbacks.
- Environment: Android emulator `WEIF_API_36` (Android 16, 720x1600 override), debug dev client, Metro on 8081, and this build of the Go server with `DEV_LOGIN_ENABLED=1` against a temporary copy of `server/event.sqlite`. Host had zero visible Discover events and nine visible notification rows. The emulator was forced to SwiftShader because of host memory pressure.
- Flow: Host → Profile → Notifications → tap the unavailable `Stale 1:1 notification QA` row → Discover empty state → `Event unavailable` result sheet → dismiss → repeat.
- Device result: PASS — repeated taps left Notifications first, kept the complete Discover empty scene mounted, and then presented the result sheet. A stressed debug run also exercised the one-shot fallback rather than losing the notice. The local resolve endpoint remained fast (roughly 2–12 ms in the final runs). SwiftShader emitted skipped-frame warnings, so this run is functional/ordering evidence only and is not a physical-device smoothness or FPS claim; no sheet spring/material constants were tuned against it.
- Automated: full frontend Jest **119 suites / 1,445 tests passed**; `npm run typecheck` passed; targeted ESLint reported 0 errors with existing warning-baseline items only; touched-file Prettier checks and `git diff --check` passed.
