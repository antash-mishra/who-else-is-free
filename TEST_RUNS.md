...
- Changes: primed invisible overlay at press-in; provider-started timings with moving phases kept in the ref and every visual rule progress-driven; takeoff on the frame after the destination mount; retained, pre-loaded return overlay; `useEventSharedTransitionMaterial` so the Android hero-button BlurViews unmount at return start without re-rendering consumers; `withSteadyTiming` bounding the first six frame steps (the display's 60→120 Hz switch otherwise became a 15–20 % first step).
...
## 2026-09-10 — Issue #127 Chat follow-up (point 1 reopened by testers)

- Change: notification taps for join requests now land on the chat screen the request belongs to (group `ChatThread`, or `OneToOneHub` for a 1:1 plan keyed by the negative event id) and raise the `JoinRequest` Requests sheet over it after the push transition (`runAfterTransition` in `src/context/pushRouting.ts`); the full-page JoinRequestScreen from 6dc762f is deleted, the sheet formerly at `PendingRequests` is renamed `JoinRequest` (route, file, `JoinRequestSheet`) with an optional conversation id, the server resolution carries `group_type`, and `OneToOneHubScreen` drops its unreachable group mode. Also: `backfillSystemMessageCopy` rewrites legacy `joined the chat` / `Updated Event Detail` system rows on startup; the 1:1 member report flow mirrors Event Details (confirm → named prompt, `getMemberReportError` copy, no raw errors); `ChatContext` splits `error` (Messages) from `threadError` (thread).
- Environment: Android emulator `WEIF_API_36` (Android 16, 720x1600 override), debug dev client `app-debug.apk` (the emulator carried a non-debuggable release APK, which had to be uninstalled first), second Metro on 8082 with `EXPO_PUBLIC_API_BASE_URL=http://10.0.2.2:8083`, this build of the Go server on `PORT=8083` with `DEV_LOGIN_ENABLED=1` against a copy of the local `event.sqlite`.
- Flow (Host via Dev Login): seeded `Issue 127 Group Hike` (Group, id 41) and `Issue 127 Coffee 1:1` (Single, id 42) plus one pending Tester request on each → Profile → bell → tap the Group row → tap the 1:1 row → accept from the sheet → open the accepted chat → header actions → Report & Block.
...
## 2026-09-24 — Event Details scroll + tab swipe stabilisation (stack gesture patch)

- Root cause: `@react-navigation/stack` 7.6.12 sets its back-swipe pan with gesture-handler v1 props (`minOffsetX: 5`, `maxDeltaY: 20`) that gesture-handler 2.28 silently drops, so the stack pan activated after touch slop in any direction and cancelled the native `ScrollView` wherever it received a touch. On Android it also received touches inside the Requests pager (RNGH measures the edge hitSlop against the overflowing two-page row; the Members page's negative offset misses the bounds, which is why only the Requests tab broke). A full-width `gestureResponseDistance` extended the same failure to the whole page.
- Change: `patches/@react-navigation+stack+7.6.12.patch` (`postinstall: patch-package`) maps the props to `activeOffsetX`/`failOffsetY` (and the vertical equivalents); Event Details keeps the 50dp edge with no edge touch layer; `HostRequestTabs` fails edge touches and `blocksExternalGesture`s the stack pan. Diagnosis on the emulator used temporary JS logs: the pager went BEGAN → FAILED on vertical motion and the `ScrollView` then received `touchCancel` without `onScrollBeginDrag`.
- Device result (WEIF_API_36, local server, `Gesture Test Hike` with 5 pending requests): PASS for every case — vertical drag from the Requests rows scrolls; vertical drag from the empty area under the Members list scrolls; interior leftward drag Requests → Members; interior rightward drag Members → Requests; interior rightward drag on Requests stays on Event Details; vertical drags starting at x=15 scroll both ways without navigating; edge swipe (x=5) returns to Discover from the details area, from the Requests rows and from the Members tab; a 70px edge drag cancels and stays; header back button returns; Create Event's full-width back swipe still closes the form.
- Automated: `npm run typecheck` clean; full Jest 118/118 suites, 1415 tests; `stackGestureActivation.test.ts` fails with the patch reversed and passes with it applied; Prettier and `git diff --check` clean on touched files.
- Not yet run: physical Galaxy A56 and iOS.

## 2026-09-24 — Event Details back zone widened to 100dp
