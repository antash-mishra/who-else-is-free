...
- `JoinRequestsScreen`, `PendingRequestsScreen`
- `PastEventsScreen`
...
- Search/filter logic lives in the shared `searchCovers` helper
  (`src/utils/coverSearch.ts`): matches tag segments and category labels,
  and appends Generic covers to every result set so the grid is never empty.
