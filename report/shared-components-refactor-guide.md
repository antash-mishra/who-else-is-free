...
- `EventDetailsScreen` keeps the route's last ready event snapshot mounted until route removal. This prevents a successful delete refresh from replacing Details with the not-found fallback before its navigation reset. A back press from a scrolled card-origin page restores the hero to the top before starting the shared return, keeping both measured endpoints valid.
