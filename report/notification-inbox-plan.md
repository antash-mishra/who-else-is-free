...
9. **Live badge, lazy list** ✅ — Foreground `onMessage` refreshes only the unread **count** (cheap `GET /api/notifications/unread-count`); the full paginated list is loaded on inbox open, pull-to-refresh, and `loadMore` (not on every `onMessage`). **Superseded (2026-09-05):** the server now emits a `notification:new` WebSocket frame for every persisted row; `NotificationsContext` prepends that single row live and bumps the count, and `NotificationBannerHost` shows a foreground banner for it. The `onMessage` count refresh remains as a reconciliation fallback; full-list refetches are still limited to inbox open, pull-to-refresh, and `loadMore`.
